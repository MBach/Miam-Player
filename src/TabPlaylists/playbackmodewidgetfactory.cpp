...
{
	if (_playbackModeButton->_mediaPlayer->playlist() == nullptr) {
		_playbackModeButton->_mediaPlayer->setPlaylist(_tabPlaylists->currentPlayList()->mediaPlaylist());
	}
	_playbackModeButton->_mediaPlayer->playlist()->setPlaybackMode(mode);
	this->update();
}
